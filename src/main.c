...
void save_window_state() {
  ...
  g_key_file_set_integer(keyfile, "WindowState", "PositionX",
                         window_state.pos_x);
  g_key_file_set_integer(keyfile, "WindowState", "PositionY",
                         window_state.pos_y);
  ...
  g_key_file_set_string(keyfile, "WindowState", "Command",
...
gboolean window_store_state(gpointer data) {
  gtk_window_get_position(GTK_WINDOW(window), &window_state.pos_x,
                          &window_state.pos_y);
  gtk_window_get_size(GTK_WINDOW(window), &window_state.width,
                      &window_state.height);
...
gboolean window_load_state(gpointer data) {
  const char *appid =
      g_application_get_application_id(g_application_get_default());
  char *file =
      g_build_filename(g_get_user_cache_dir(), appid, "state.ini", NULL);
  GKeyFile *keyfile = g_key_file_new();
  ...
  error = NULL;
  window_state.pos_x =
      g_key_file_get_integer(keyfile, "WindowState", "PositionX", &error);
  if (error != NULL) {
    window_state.pos_x = 90;
  }
  error = NULL;
  window_state.pos_y =
      g_key_file_get_integer(keyfile, "WindowState", "PositionY", &error);
  if (error != NULL) {
    window_state.pos_x = 90;
  }
  error = NULL;
  window_state.width =
      g_key_file_get_integer(keyfile, "WindowState", "Width", &error);
  if (error != NULL) {
    window_state.width = 200;
  }
  error = NULL;
  window_state.height =
      g_key_file_get_integer(keyfile, "WindowState", "Height", &error);
  if (error != NULL) {
    window_state.height = 120;
  }
  error = NULL;
  window_state.command =
      g_key_file_get_string(keyfile, "WindowState", "Command", &error);
  if (error != NULL) {
    window_state.command = "cat $file$";
  }

  gtk_window_move(GTK_WINDOW(window), window_state.pos_x, window_state.pos_y);
  gtk_window_resize(GTK_WINDOW(window), window_state.width,
                    window_state.height);
...
void window_destroy(GtkWidget *widget, gpointer data) { save_window_state(); }

#define UI_DRAG_TARGETS_COUNT 3

enum { DT_TEXT, DT_URI, DT_URI_LIST };
...
gpointer worker(gpointer data) {
  ...
  for (int i = 0; workerData->files[i] != NULL; i++) {
    ...
    const gchar *execute =
        g_strjoinv(file, g_strsplit(workerData->command, "$file$", -1));
    g_print("Executing: '%s'\n", execute);
...
void on_drag_data_received(GtkWidget *widget, GdkDragContext *context, int x,
                           int y, GtkSelectionData *data, guint info,
                           guint time) {
  guchar *text = NULL;
  gchar **files = NULL;
...
gboolean entry_tooltip(GtkWidget *widget, gint x, gint y, gboolean keyboard_tip,
  ...
  gtk_tooltip_set_text(tooltip,
                       "Use $file$ to replace with the dropped file path.");
  return TRUE;
}
...
void activate(GtkApplication *app, gpointer user_data) {
  ...
  g_signal_connect(G_OBJECT(window), "destroy", G_CALLBACK(window_destroy),
                   NULL);
  ...
  gtk_drag_dest_set(label, GTK_DEST_DEFAULT_ALL, ui_drag_targets,
                    UI_DRAG_TARGETS_COUNT, GDK_ACTION_LINK);
  g_signal_connect(label, "drag-data-received",
  ...
  g_signal_connect(GTK_ENTRY(entry), "query-tooltip", G_CALLBACK(entry_tooltip),
                   NULL);
